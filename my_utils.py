...
def take_item(player_inventory, room_inventory, item):
    if has_a(room_inventory, item):
        ...
        if has_a(player_inventory, item):
            ...
            player_inventory = player_count + 1
        else:
...
def make_visited(rooms_visited, room_number):
    rooms_visited.append(room_number)
    return rooms_visited

def room_visited_check(rooms_visited, number):
    if number in rooms_visited:
        room_visited = True
    else:
        room_visited = False
    return room_visited
